...
import { create_rolls, createPosition_API, getPosition_API, updatePosition_API } from '../../api_services/Apiservices'
import { useNavigate } from 'react-router-dom'
import { TbEdit } from 'react-icons/tb'
import Swal from 'sweetalert2'
import { MyInputField } from '../../components/MyInputField'

export const PosiotinForm = ({ setLoder, positionOp, getposition }) => {
    ...
    const createHandler = async () => {
        if (fdata && !fdata.name) { errorAlert("Please Enter Position"); return null; }
        setLoder(true)
        const datas = { "name": fdata.name, }
        const resp = await createPosition_API(datas);
    ...
    const updateHandler = async () => {
        ...
        const resp = await updatePosition_API(datas);
        if (resp && resp.success) {
            setLoder(false);
            setIsedit(false);
            setFdata((pre) => ({ ...pre, "name": "" }));
        ...
        <>
            <div className='CreateRoleForm'>
                <Container>
                    ...
                    <Row>
                        <Col md={6}>
                            <MyInputField required={true} FormLabel='Position Name' FormPlaceHolder='Enter Position Name' value={fdata.name} onChange={(e) => setFdata((pre) => ({ ...pre, 'name': e.target.value }))} />
                        </Col>
                        {isedit ?
                            <Col md={6} >
